...
class FullHouse extends Rule {
  evalRoll = dice => {
    let firstEval = this.freq(dice).toString() === [2, 3].toString() ? this.score : 0;
    let secondEval = this.freq(dice).toString() === [3, 2].toString() ? this.score : 0;
...
class SmallStraight extends Rule {
  evalRoll = dice => {
    ...
    let firstEval = d.includes(1) && d.includes(2) && d.includes(3) && d.includes(4);
    let secondEval = d.includes(2) && d.includes(3) && d.includes(4) && d.includes(5);
    let thirdEval = d.includes(3) && d.includes(4) && d.includes(5) && d.includes(6);

    // small straight must be 4 different dice in a row
    return firstEval || secondEval || thirdEval ? this.score : 0;
  };
}
